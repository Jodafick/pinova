...
function cardRingClass(rank: PodiumRank): string {
  if (rank === 1)
    return 'text-[#F5D48A] dark:text-[#FFDF99] ring-[rgba(200,146,42,0.3)] dark:ring-[rgba(255,210,120,0.55)] bg-[rgba(200,146,42,0.12)] dark:bg-[rgba(255,200,90,0.2)]'
  if (rank === 2)
    return 'text-[#C8D3DC] dark:text-[#F0F6FF] ring-[rgba(142,155,170,0.25)] dark:ring-[rgba(200,215,235,0.55)] bg-[rgba(142,155,170,0.1)] dark:bg-[rgba(190,205,225,0.18)]'
  return 'text-[#C89B6E] dark:text-[#FFCF94] ring-[rgba(139,90,43,0.25)] dark:ring-[rgba(255,180,120,0.5)] bg-[rgba(139,90,43,0.1)] dark:bg-[rgba(230,150,95,0.18)]'
}

function scoreClass(rank: PodiumRank): string {
  if (rank === 1) return 'text-[#F5D48A] dark:text-[#FFE9A8]'
  if (rank === 2) return 'text-[#C8D3DC] dark:text-[#F8FAFC]'
  return 'text-[#C89B6E] dark:text-[#FFD2A8]'
}
...
function cardBorderClass(rank: PodiumRank, you: boolean): string {
  const base =
    'rounded-2xl rounded-b-none border bg-[#131519] dark:bg-[#131519] transition duration-200 hover:-translate-y-1'
  const youRing = you ? ' ring-2 ring-pink-500/75 ring-offset-2 ring-offset-[#0c0d0f]' : ''
  if (rank === 1)
    return `${base} border-[rgba(200,146,42,0.3)] dark:border-[rgba(255,205,110,0.55)] shadow-[0_0_40px_rgba(200,146,42,0.08)] dark:shadow-[0_0_48px_rgba(255,200,80,0.18)]${youRing}`
  if (rank === 2)
    return `${base} border-[rgba(142,155,170,0.25)] dark:border-[rgba(210,225,245,0.45)] shadow-[0_0_30px_rgba(142,155,170,0.05)] dark:shadow-[0_0_40px_rgba(200,215,235,0.12)]${youRing}`
  return `${base} border-[rgba(139,90,43,0.25)] dark:border-[rgba(255,175,110,0.5)] shadow-[0_0_30px_rgba(139,90,43,0.05)] dark:shadow-[0_0_42px_rgba(255,150,80,0.14)]${youRing}`
}
</script>
...
<template>
  <section
    v-if="showPodium"
    class="relative z-[1] w-full max-w-[49rem] mx-auto mb-6 sm:mb-8 px-1 sm:px-2 text-[#F0EDE8]"
    aria-label="Top 3"
  >
    <div
    ...
    <header class="podium-fade mb-6 sm:mb-8 text-center podium-fade-header">
      ...
      <h2
        class="podium-bebas text-[clamp(2.5rem,7vw,4.25rem)] leading-none tracking-[0.05em] text-[#F5D48A] dark:text-[#FFE9A8] sm:bg-gradient-to-br sm:from-white sm:via-white sm:to-[#F5D48A] sm:dark:from-[#FFFDF8] sm:dark:via-[#FFEFC0] sm:dark:to-[#FFD875] sm:bg-clip-text sm:text-transparent"
      >
      ...
      >
        ...
        >
          ...
          <template v-if="col.row">
            ...
            <p
              class="relative z-[1] mb-2 mt-2 podium-dm text-[10px] font-light uppercase tracking-[0.18em] text-[rgba(240,237,232,0.45)]"
            >
              {{ placeLabel(col.podiumRank) }}
              <span
                v-if="isYouRow(col.row)"
                class="ml-1.5 rounded-full bg-pink-500/25 px-1.5 py-px text-[9px] font-semibold text-pink-200"
              >
                {{ t('contest.live.youBadge') }}
              </span>
            </p>
            <p
              class="relative z-[1] min-h-[2.125rem] podium-dm text-[11px] font-medium leading-snug text-[#F0EDE8] sm:text-[13px] line-clamp-2 px-0.5"
            >
              {{ pinTitle(col.row) }}
            </p>
            <p
              v-if="pinSubtitle(col.row)"
              class="relative z-[1] mb-3.5 mt-1 podium-dm text-[11px] font-light text-[rgba(240,237,232,0.45)] truncate px-0.5"
            >
              {{ pinSubtitle(col.row) }}
            </p>
            ...
            <div class="relative z-[1] mb-3">
              ...
              <p class="podium-dm text-[10px] font-light uppercase tracking-[0.2em] text-[rgba(240,237,232,0.45)]">
            ...
            <div v-else-if="col.row && variant === 'referral'" class="relative z-[1] flex justify-center">
              <span class="flex items-center gap-1 podium-dm text-[11px] font-light text-[rgba(240,237,232,0.45)]">
            ...
            <span
              ...
              :class="
                trendBadgeTone(col.row) === 'neutral'
                  ? 'border border-[rgba(255,255,255,0.06)] bg-[rgba(255,255,255,0.04)] text-[rgba(240,237,232,0.45)]'
                  : trendBadgeTone(col.row) === 'up'
                    ? 'border border-[rgba(59,109,17,0.3)] bg-[rgba(59,109,17,0.18)] text-[#7EC850]'
                    : 'border border-[rgba(180,50,50,0.35)] bg-[rgba(180,50,50,0.15)] text-[#f87171]'
              "
            >
          ...
          <template v-else>
            <div class="py-10 podium-dm text-sm text-[rgba(240,237,232,0.28)]">
              —
            </div>
          </template>
        </component>
        <div class="relative z-[1] w-full flex items-center justify-center" :class="podiumBlockClass(col.podiumRank)">
          <span class="podium-bebas text-[1.75rem] text-white/20">{{ col.podiumRank }}</span>
        </div>
      </div>
    </div>
  </section>
