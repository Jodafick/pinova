  ...
  <div class="min-h-screen flex items-center justify-center bg-neutral-50 px-6 py-12">
    <div class="w-full max-w-md bg-white p-8 sm:p-10 rounded-[40px] shadow-sm border border-neutral-100">
      <div class="text-center mb-8">
        ...
        <h2 class="text-3xl font-extrabold text-neutral-900 mb-2">Mot de passe oublié ?</h2>
        <p class="text-neutral-500">Pas de panique, ça arrive ! Entrez votre email pour recevoir un lien de réinitialisation.</p>
      </div>
      ...
      <div v-if="success" class="bg-green-50 border border-green-100 rounded-2xl p-6 text-center animate-fade-in">
        ...
        <h3 class="text-green-800 font-bold mb-1">Email envoyé !</h3>
        <p class="text-green-700 text-sm">Consultez votre boîte de réception pour réinitialiser votre mot de passe.</p>
        <router-link to="/login" class="inline-block mt-6 text-green-800 font-bold hover:underline">Retour à la connexion</router-link>
      </div>
      ...
      <form v-else @submit.prevent="handleForgotPassword" class="space-y-6">
        ...
        <p class="text-center">
          <router-link to="/login" class="text-sm font-bold text-neutral-500 hover:text-neutral-700">Annuler et revenir</router-link>
        </p>
      </form>
    </div>
  </div>
