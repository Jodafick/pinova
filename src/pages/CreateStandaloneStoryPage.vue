...
<template>
  ...
  <div
    ...
    :class="[
      isLgDown
        ? 'flex min-h-0 w-full flex-1 flex-col overflow-hidden overflow-x-hidden bg-[#060408] text-white'
        : 'story-create-shell flex w-full flex-1 flex-col min-h-0 bg-[#060408] text-white overflow-hidden',
    ]"
  >
    <input
      ref="galleryInput"
    ...
    <div
      v-if="step === 'caption'"
      :class="[
        'story-caption-step relative flex min-h-0 flex-1 flex-col px-5 pb-[calc(env(safe-area-inset-bottom,0px)+1.25rem)] pt-[calc(env(safe-area-inset-top,0px)+1rem)]',
        isLgDown ? '' : 'min-h-0 flex-1',
      ]"
    >
    ...
    <div
      v-else-if="step === 'pick'"
      :class="[
        'story-media-step relative flex min-h-0 flex-1 flex-col px-5 pb-[calc(env(safe-area-inset-bottom,0px)+1.25rem)] pt-[calc(env(safe-area-inset-top,0px)+0.75rem)]',
        isLgDown ? '' : 'min-h-0 flex-1',
      ]"
    >
      <div class="pointer-events-none absolute -right-16 -top-20 h-64 w-64 rounded-full bg-pink-700/10 dark:bg-pink-600/10 blur-2xl" />
