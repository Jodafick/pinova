...
onMounted(async () => {
  ...
  try {
    ...
    error.value = t('verifyEmail.error.desc')
  } finally {
    loading.value = false
  }
})
  ...
  <div class="min-h-screen flex items-center justify-center bg-neutral-50 px-6 py-12">
    <div class="w-full max-w-md bg-white p-8 sm:p-10 rounded-[40px] shadow-sm border border-neutral-100 text-center">
      <div v-if="loading">
        ...
        <h2 class="text-2xl font-bold text-neutral-900 mb-2">{{ t('verifyEmail.loading.title') }}</h2>
        <p class="text-neutral-500">{{ t('verifyEmail.loading.desc') }}</p>
      </div>
      ...
      <div v-else-if="success" class="animate-fade-in">
        ...
        <h2 class="text-2xl font-bold text-neutral-900 mb-2">{{ t('verifyEmail.success.title') }}</h2>
        <p class="text-neutral-500">{{ t('verifyEmail.success.desc') }}</p>
      ...
      <div v-else class="animate-fade-in">
        ...
        <h2 class="text-2xl font-bold text-neutral-900 mb-2">{{ t('verifyEmail.error.title') }}</h2>
