...
/**
 * Verrouillage du scroll `body` partagé par toutes les instances de `FotoceModal`.
...
export function acquireFotoceBodyScrollLock(): void {
  if (typeof document === 'undefined') return
  if (lockCount === 0) {
    savedScrollY = getAppScrollRoot().scrollTop
    document.body.classList.add('fotoce-modal-scroll-lock')
  }
  lockCount += 1
}

export function releaseFotoceBodyScrollLock(): void {
  if (typeof document === 'undefined') return
  lockCount = Math.max(0, lockCount - 1)
  if (lockCount === 0) {
    document.body.classList.remove('fotoce-modal-scroll-lock')
...
export function resetFotoceBodyScrollLock(): void {
  if (typeof document === 'undefined') return
  lockCount = 0
  document.body.classList.remove('fotoce-modal-scroll-lock')
}
